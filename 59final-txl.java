//// Teng Lin, Final CST112, 12/10/2015
...
int lint=4;
Lobster [] txl = new Lobster[lint];   //array of lobster object
...
void reset() {
  ...
  txl[0]= new Lobster( random(surface+150, height), random(1,3), random (1,3));
 
 
  //lobster
  for (int i=1; i<lint; i++) {
  txl[i]= new Lobster( random(surface+150, height), random(1,3), random (1,3));
  }
...
void messages() {
  ...
  text( "Pressed r key to reset.", width/3, 80 );
  text( "Teng Lin:  Final", 10, height-10 );
...
void scene() {
 ...
 /*
  //red triangles along the bottom.
  for (int i=0; i<width; i++) {
    fill(255,0,0);
    triangle( i, height, i+30, height, i+15, height-20);
    i=i+50;
  }
 */
//loop didnt work.

 for (float i=surface; i<height; i++) {
    fill(255,0,0);
    triangle( width,surface, width, surface+30, width-20, surface+15);
    i=i+50;
 }
  triangle( width,surface+50, width, surface+80, width-20, surface+65);
  triangle( width,surface+100, width, surface+130, width-20, surface+115);
  triangle( width,surface+150, width, surface+180, width-20, surface+165);
  triangle( width,surface+200, width, surface+230, width-20, surface+215);
  triangle( width,surface+250, width, surface+280, width-20, surface+265);
  triangle( width,surface+300, width, surface+330, width-20, surface+315);
  triangle( width,surface+350, width, surface+380, width-20, surface+365);
...
void lobster(){
 ...
 //lobsters bounce off each other
 collision( txl[0],txl[1]);
 collision( txl[0],txl[2]);
 collision( txl[0],txl[3]);

 collision( txl[1],txl[2]);
 collision( txl[1],txl[3]);
 
 collision( txl[2],txl[3]);
 
 //lobster catching the squid.
 for (int i=0; i<lin; i++){
 if (  dist( txl[0].x,txl[0].y, school[i].x,school[i].y ) < 30 ){
   school[i].bottom();
  }
 }
 for (int i=0; i<lin; i++){
 if (  dist( txl[1].x,txl[1].y, school[i].x,school[i].y ) < 30 ){
   school[i].bottom();
  }
 }
 for (int i=0; i<lin; i++){
 if (  dist( txl[2].x,txl[2].y, school[i].x,school[i].y ) < 30 ){
   school[i].bottom();
  }
 }
 for (int i=0; i<lin; i++){
 if (  dist( txl[3].x,txl[3].y, school[i].x,school[i].y ) < 30 ){
   school[i].bottom();
  }
 }

}
void collision( Lobster p, Lobster q ) {
 if ( p.hit( q.x,q.y ) ) {
    float tmp;
...
class Lobster {
 float x=random(50,300), y;
 float dx, dy;
 float r=(random(0,255)), b=(random(0,255)), g=(random(0,255));
  
  
 //// CONSTRUCTORS ////
 Lobster( float tempy, float tempdy, float tempdx) {
   y=tempy;
   dy=tempdy;
   dx=tempdx;
 }
  ...
  void show() {
    ...
    if (k==0) {
    stroke(5);
    line(x+13, y-15, x+40, y-15);
    line(x+13, y+15, x+40, y+15);
    }
    else{
    stroke(5);
    line(x+13, y-15, x+40, y-5);
    line(x+13, y+15, x+40, y+5);
    } 
    }
    else{
  ...
  boolean hit( float x, float y ) {
    if (  dist( x,y, this.x,this.y ) < 40 ) return true;
    else return false;
  }
 void move() {
